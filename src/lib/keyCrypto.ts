...
const _poolCache = new Map<string, string[]>();

/** Store an API key pool (string[]) encrypted in localStorage. */
export async function setEncryptedPool(storageKey: string, pool: string[]): Promise<void> {
  if (!pool || pool.length === 0) {
    localStorage.removeItem(storageKey);
    _poolCache.delete(storageKey);
    return;
  }
  _poolCache.set(storageKey, pool);
  const encrypted = await encryptValue(JSON.stringify(pool));
  localStorage.setItem(storageKey, encrypted);
}

/** Read an encrypted API key pool from localStorage (async). */
export async function getEncryptedPool(storageKey: string): Promise<string[]> {
  const cached = _poolCache.get(storageKey);
  if (cached) return cached;
  const stored = localStorage.getItem(storageKey);
  if (!stored) return [];
  try {
    const plaintext = await decryptValue(stored);
    if (!plaintext) return [];
    const pool = JSON.parse(plaintext) as string[];
    _poolCache.set(storageKey, pool);
    return pool;
  } catch {
    return [];
...
/** Synchronous pool read — from memory cache or legacy plaintext JSON. */
export function getApiKeyPool(storageKey: string): string[] {
  const cached = _poolCache.get(storageKey);
  if (cached) return cached;
  const stored = localStorage.getItem(storageKey);
  if (!stored) return [];
