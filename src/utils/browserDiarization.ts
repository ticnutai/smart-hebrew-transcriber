...
/** Extract rich features: MFCC + spectral contrast + ZCR + energy */
function extractRichFeatures(
  frame: Float64Array,
  ...
  // Concat: MFCC(20) + contrast(6) + zcr(1) + energy(1) = 28 dims
...
/** Augment with Δ and ΔΔ features */
...
/** Estimate speaker count using silhouette score + BIC */
function estimateSpeakerCount(vectors: Float64Array[]): number {
  ...
  for (let k = 2; k <= maxK; k++) {
    ...
    // Combine silhouette with penalty for too many clusters
...
function detectVoiceActivity(energies: number[]): boolean[] {
  ...
  // Apply hangover: keep active for a few frames after voice stops
  const hangover = 3;
  const result = [...active];
...
export async function diarizeInBrowser(
  ...
  // Pre-emphasis filter to boost high frequencies (improves speaker discrimination)
  ...
  onProgress?.({ stage: "מחלץ מאפיינים מתקדמים...", percent: 12 });
  ...
  for (let i = 0; i < totalFrames; i++) {
    ...
    if (i % 800 === 0) {
      onProgress?.({ stage: "מחלץ מאפיינים מתקדמים...", percent: 12 + (i / totalFrames) * 28 });
      await new Promise(r => setTimeout(r, 0));
    }
  }

  onProgress?.({ stage: "מחשב מאפייני דלתא ותאוצה...", percent: 42 });

  // Augment with Δ and ΔΔ: 28 * 3 = 84 features
  const augmented = augmentWithDeltaAndAccel(frameFeatures);
  const augDim = augmented[0]?.length || 84;

  onProgress?.({ stage: "מנתח חלונות זמן...", percent: 50 });
  ...
  // Overlapping windows (50% overlap) for smoother transitions
  ...
  for (let w = 0; w < totalFrames - framesPerWindow; w += windowStep) {
    ...
    // Also compute variance for richer representation
    ...
    // Combined vector: mean + std = 168 dims
  ...
  // Adaptive VAD
  onProgress?.({ stage: "מזהה קטעי דיבור...", percent: 58 });
  ...
  onProgress?.({ stage: "מנרמל מאפיינים...", percent: 63 });
  const normalizedVectors = normalizeFeatures(activeVectors);

  onProgress?.({ stage: "מזהה מספר דוברים...", percent: 68 });
  const numSpeakers = expectedSpeakers || estimateSpeakerCount(normalizedVectors);

  onProgress?.({ stage: `מקבץ ל-${numSpeakers} דוברים...`, percent: 75 });
  ...
  // Fill silence with nearest active speaker
  ...
  onProgress?.({ stage: "מחליק מעברים...", percent: 88 });

  // Two-pass smoothing: median filter then minimum-duration filter
  const smoothedLabels = [...allLabels];
  
  // Pass 1: Majority vote with window=5
  const smoothW = 2;
  ...
  // Pass 2: Remove very short speaker segments (< 3 windows)
  const minWindows = 3;
  let runStart = 0;
  ...
  // Merge short segments with same neighbor
  ...
  audioCtx.close();
  onProgress?.({ stage: "הושלם!", percent: 100 });
